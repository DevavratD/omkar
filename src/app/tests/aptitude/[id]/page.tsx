import AptitudeClientWrapper from './AptitudeClientWrapper';
...
export default async function AptitudeTestPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // STRICT ISOLATION: Provide aptitude questions along with profiling meta questions
  const aptitudeQuestions = questions.filter(q => q.type === 'aptitude' || q.type === 'meta');
    ...
    return (
      <div className="flex h-screen items-center justify-center bg-slate-50">
        <div className="text-center space-y-4">
          ...
          <p className="text-slate-500 max-w-md">No aptitude questions found in the database. Please run the seeder.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <main className="min-h-screen bg-white">
      <AptitudeClientWrapper testId={id} questions={aptitudeQuestions} />
    </main>
  );
}
